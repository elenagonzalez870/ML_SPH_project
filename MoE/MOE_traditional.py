...
class MixtureOfExpertsLayer(nn.Module):
    """MoE layer with multiple experts and a router"""
    ...
    def forward(self, x):
        ...
        gates, top_k_indices = self.router(x) 
        ...
        # Auxiliary loss to prevent all examples going to same expert
        importance = gates.sum(0)  
        load_loss = (importance.float().var() / (importance.float().mean() + 1e-10))
...
class Router(nn.Module):
    ...
    def forward(self, x):
        ...
        # Create full gate tensor (for easier computation)
...
class TaskSpecificMoE(nn.Module):
    ...
    def __init__(self, num_experts=4, top_k=2):
        ...
        # Separate MoE for classification
        self.class_moe = MixtureOfExpertsLayer(
            input_dim=256,
            hidden_dim=128,
        ...
        # Separate MoE for regression
        self.reg_moe = MixtureOfExpertsLayer(
            input_dim=256,
            hidden_dim=128,
            num_experts=num_experts,
            top_k=top_k
    ...
    def forward(self, x):
        ...
        # Classification path
        class_features, class_load_loss = self.class_moe(shared_features)
        class_out = self.class_head(class_features)
        
        # Regression path
        reg_features, reg_load_loss = self.reg_moe(shared_features)
        reg_out = self.reg_head(reg_features)
        reg_out_fractions = F.softmax(reg_out, dim=-1)
        
        total_load_loss = class_load_loss + reg_load_loss
        
        return class_out, reg_out_fractions, total_load_loss
...
def train(dataloader, model, loss_fn, reg_loss_fn, optimizer, train_mean, train_std, cfg, awl, device, load_loss_weight=0.01):
    ...
    for X, (true_class, true_reg) in dataloader:
        ...
        pred_class, pred_reg, load_loss = model(X)
        ...
        # Combining the loss 
        task_loss = awl(loss_class, loss_reg)
        loss = task_loss + load_loss_weight * load_loss  # Add load balancing loss
...
def main():
    ...
    run_id = wandb.run.id
    wandbname = f"MoE_{date_str}_{run_id}"
