...
def save_results(grid_search, scaler, predictions, output_dir, results_dir):
    ...
    # Save best model
    model_path = os.path.join(output_dir, 'svr_best_model.pkl')
    joblib.dump(grid_search.best_estimator_, model_path)
    print(f"Saved best model to {model_path}")
    
    # Save scaler
    scaler_path = os.path.join(output_dir, 'svr_scaler.pkl')
    ...
    results_path = os.path.join(output_dir, 'svr_training_info.pkl')
    joblib.dump(results, results_path)
    print(f"Saved training info to {results_path}")
    
    # Save predictions
    y_pred_train, y_pred_val, y_pred_test = predictions
    predictions_path = os.path.join(results_dir, 'svr_results.npz')
    np.savez(
        predictions_path,
